...
class WaveReco:
    ...
    class WaveBox_t():
        ...
        def BookInput(self, tin, evfrom=0, maxev=-1, isarray=0):
            self.waveIn = []
            # for ii in (self.inWvs):
            for ii in WaveReco.ttqdm(self.inWvs, "blue"):
                # end = evfrom + maxev if maxev > 0 else -1
                # self.waveIn.append(tin[ii].array(library="np")[evfrom:end, :])
                if isarray: self.waveIn.append(tin[ii])
                else: self.waveIn.append(tin[ii].array(library="np"))
    ...
    def Book(self):
        ...
        if self.evtBranch != None:
            self.inb_evt = self.inTree[self.evtBranch]
        
        for ii in WaveReco.ttqdm(self.wboxes, "magenta"):
            ii.BookInput(self.inTree, isarray=self.inTreeIsArray)
    ...
    def Launch(self):
        ...
        # self.entries = self.inTree.num_entries
    ...
    def LoopEntries(self):
        ...
        for ientry in (WaveReco.ttqdm(range(self.etp))): 
            ...
            evnum = ientry if self.evtBranch == None else self.inb_evt[ientry]
            self.b_evt[0] = evnum
            ...
            for ibox, box in enumerate(self.wboxes):
                ...
                for iwave, win in enumerate(box.waveIn):
                    ...
                    if box.specSaved < self.specsToSave and self.anaFileName != None:
                        ...
                        title = F"ev{evnum}_{outname}{iwaveout}"
                        ...
                        gra.SetTitle(F"event: {evnum}   chan: {outname}{iwaveout};tim [ns]; amp [mV]")
